...
public class Benchmark
{
    ...
    public static IEnumerable<object[]> EnumerableItemsToFind()
    {
        ...
        {
            Enumerable.Range(0, 5).OrderBy(x => x % 3).ToArray(),
            Enumerable.Range(0, 10).OrderBy(x => x % 2)
        };
        yield return new object[]
        {
            Enumerable.Range(0, 50).OrderBy(x => x % 3).ToArray(),
            Enumerable.Range(0, 100).OrderBy(x => x % 2)
        };
        yield return new object[]
        {
            Enumerable.Range(0, 500).OrderBy(x => x % 3).ToArray(),
    ...
    public void RunSearchForSet(int[] itemsToFind, IEnumerable<int> items)
    {
        ...
        for (var j = 0; j < itemsToFind.Length; j++) _contains = set.Contains(itemsToFind[j]);
    }
    ...
    {
        for (var j = 0; j < itemsToFind.Length; j++) _contains = items.Contains(itemsToFind[j]);
    }
    ...
    public void RunSearchForPreCreatedList(int[] itemsToFind, IEnumerable<int> items)
    {
        ...
        for (var j = 0; j < itemsToFind.Length; j++) _contains = list.Contains(itemsToFind[j]);
    }
