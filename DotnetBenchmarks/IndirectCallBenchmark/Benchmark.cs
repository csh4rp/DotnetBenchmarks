...
public class Benchmark
{
    ...
    {
        for (var i = 0; i < NumberOfIterations; i++) _result = Calculator.Add(X, Y);
    }

    [Benchmark(Description = "Emitted delegate call")]
    public void RunEmittedDelegateCall()
    {
        for (var i = 0; i < NumberOfIterations; i++) _result = EmittedDelegate(Calculator, X, Y);
    }

    [Benchmark(Description = "Dynamic call")]
    public void RunDynamicCall()
    {
        for (var i = 0; i < NumberOfIterations; i++) _result = ((dynamic)BoxedCalculator).Add(X, Y);
    }
